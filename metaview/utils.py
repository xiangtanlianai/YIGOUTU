...
def setup_log_dir(args, sampling=False):
    ...
    log_dir = os.path.join(
        args["log_dir"], "{}_{}".format(args["dataset"], date_postfix)      # 别看错了，这里合并的是两个，第二个是format
    )
...
def load_acm(remove_self_loop):
    ...
    download(_get_dgl_url(url), path=data_path)     # 系统自带函数

    with open(data_path, "rb") as f:
        data = pickle.load(f)
    # data中的字典名：label,feature,PAP,PLP,train_idx,val_idx,test_idx
    # labels, features = (
    #     torch.from_numpy(data["label"].todense()).long(),       # 转为密集矩阵
    #     torch.from_numpy(data["feature"].todense()).float(),    # 转为密集矩阵
    # )

    labels = torch.from_numpy(data["label"].todense()).long()
    ...
    labels = labels.nonzero()[:, 1]     # 找出每行不为0的索引，输出结果是（标号，索引）所以要加[:,1]来取第二列
    ...
    train_idx = torch.from_numpy(data["train_idx"]).long().squeeze(0)       # 600
    val_idx = torch.from_numpy(data["val_idx"]).long().squeeze(0)       # 300
    test_idx = torch.from_numpy(data["test_idx"]).long().squeeze(0)     # 2125

    num_nodes = author_g.number_of_nodes()      # 3025
...
def load_data_HAN(dataset, remove_self_loop=False):
    if dataset == "ACM":
