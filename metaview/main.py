...
def main(args):
    ...
    (
        ...
        val_mask,  # 验证标签
        test_mask,  # 测试标签
    ) = load_data(args["dataset"])  # 加载以及处理数据集
    ...
    if args["hetero"]:
        ...
        model = HAN(
            num_meta_paths=len(g),  # 元路径数为2
            in_size=features.shape[1],  # 1870
            hidden_size=args["hidden_units"],
            out_size=num_classes,  # 3
            num_heads=args["num_heads"],
            dropout=args["dropout"],
        ).to(args["device"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hetero", action="store_true", default=True,help="Use metapath coalescing with DGL's own dataset")
    args = parser.parse_args().__dict__  # {'seed': 1, 'log_dir': 'results', 'hetero': False}

    args = setup(args)  # 里面封装的函数
