...
class SRHGNLayer(nn.Module):
    def __init__(self,
        ...
        self.num_types = len(node_dict)  # 节点类型数
        self.num_relations = len(edge_dict)  # 边类型数

        self.total_rel = self.num_types * self.num_relations * self.num_types
        self.num_node_heads = num_node_heads
        self.num_type_heads = num_type_heads

        self.node_linear = nn.ModuleList()  # 节点线性层
        self.edge_linear = nn.ModuleList()  # 边线性层
        ...
        for _ in range(self.num_types):  # 3个节点线性层
            self.node_linear.append(nn.Linear(input_dim, output_dim))

        for _ in range(self.num_relations):  # 4个边关系线性层
        ...
        self.rel_emb = nn.Parameter(torch.randn(self.num_relations, output_dim), requires_grad=True)  # 关系嵌入
        ...
        self.epsilon = nn.Parameter(torch.FloatTensor([1e-9]), requires_grad=False)  # 探索率
    ...
    def forward(self, G, h):
        with G.local_scope():
            ...
            for src, e, dst in G.canonical_etypes:  # 三元组的形式返回图中定义的边
                # Extract subgraph for each relation  根据边关系摘出对应的子图
                sub_graph = G[src, e, dst]
                h_src = h[src]  # 源节点
                h_dst = h[dst]  # 目标节点

                e_id = edge_dict[e]  # 边类型的ID
                src_id = node_dict[src]  # 源节点ID
                dst_id = node_dict[dst]  # 目标节点ID

                # 根据关系类型将源节点映射到目标节点的空间
                h_src = self.drop(self.edge_linear[e_id](h_src))

                # 根据目标节点类型在高级空间中映射目标节点
...
class SRHGN(nn.Module):
    def __init__(self,
        ...
        for ntype, idx in node_dict.items():
            self.pre_transform.append(nn.Linear(input_dims[ntype], hidden_dim))  # 输出256  # 3个线性层
        # 卷积层
    ...
    def forward(self, G, target):
        ...
        for ntype in G.ntypes:
            ...
            h[ntype] = F.gelu(h[ntype])  # 高斯误差激活函数
...
class GAT(nn.Module):
    def __init__(self,
        ...
        if self.aggregate == 'mean':
            ...
            self.out_concat_x2_1 = nn.Linear(self.hidden_dim * self.nheads * nums_head[0],
    ...
    def forward(self, G, target, features):
        ...
        if self.aggregate == 'mean':
            ...
            x1 = F.elu(self.out_mean_x1(x1))
            x2 = F.elu(self.out_mean_x2(x2))
            xlist = F.elu(torch.add(x1, x2))

            return F.log_softmax(xlist, dim=1), h1, attns
            ...
            x2 = torch.cat(x2, dim=1)
            x1 = F.dropout(x1, self.dropout, training=self.training)
            x1 = F.elu(self.out_concat_x1(x1))
            x2 = F.elu(self.out_concat_x2_1(x2))
            x2 = F.elu(self.out_concat_x2_2(x2))
            xlist = F.elu(torch.add(x1, x2))
            ...
            x1 = F.dropout(x1, self.dropout, training=self.training)
            x1 = F.elu(self.out_max_x1(x1))
            x2 = F.elu(self.out_max_x2(x2))
            xlist = F.elu(torch.add(x1 * self.parameter1, x2 * (1 - self.parameter1)))
...
class GAT(nn.Module):
    ...
    def forward(self, G, target, features):
        ...
        if self.aggregate == 'mean':
            ...
            #     else:
            #         x2 = torch.add(x2, h2)
            # x1 = torch.zeros(G[target][0],self.hidden_dim)
            # x2 = torch.zeros(G[target][0],self.hidden_dim)
